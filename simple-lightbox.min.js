!function(ot, st, lt) {
    ...
    ot.fn.simpleLightbox = function(N) {
        ...
        function h() {
            ...
            var t = "pid=" + (V + 1)
              , e = s.href.split("#")[0] + "#" + t;
            o ? history[f ? "replaceState" : "pushState"]("", lt.title, e) : f ? s.replace(e) : s.hash = t,
            f = !0
        }
        function t(e, a) {
            var n;
        ...
        function a(t) {
            t.trigger(ot.Event("show.simplelightbox")),
            N.disableScroll && (u = T("hide")),
            N.htmlClass && "" != N.htmlClass && ot("html").addClass(N.htmlClass),
            b.appendTo("body"),
            _.appendTo(b),
            N.overlay && l.appendTo(ot("body")),
            m = !0,
            V = $.index(t),
            K = ot("<img/>").hide().attr("src", t.attr(N.sourceAttr)).attr("data-scale", 1).attr("data-translate-x", 0).attr("data-translate-y", 0),
            -1 == d.indexOf(t.attr(N.sourceAttr)) && d.push(t.attr(N.sourceAttr)),
            _.html("").attr("style", ""),
            K.appendTo(_),
            C(),
            l.fadeIn("fast"),
            ot(".sl-close").fadeIn("fast"),
            g.show(),
            J.fadeIn("fast"),
            ot(".sl-wrapper .sl-counter .sl-current").text(V + 1),
            c.fadeIn("fast"),
            tt(),
            N.preloading && w(),
            setTimeout(function() {
        ...
        var p, e, H = "ontouchstart"in st, L = (st.navigator.pointerEnabled || st.navigator.msPointerEnabled,
        0), j = 0, K = ot(), i = function() {
        ...
        ot(this).filter(function() {
            return ot(this).attr("rel") === e
        })) : this, n = $.get()[0].tagName, u = (i = i(),
        0), B = !1 !== i, o = "pushState"in history, f = !1, s = st.location, Q = P(), G = "simplelb", l = ot("<div>").addClass("sl-overlay"), r = ot("<button>").addClass("sl-close").html(N.closeText), g = ot("<div>").addClass("sl-spinner").html("<div></div>"), J = ot("<div>").addClass("sl-navigation").html('<button class="sl-prev">' + N.navText[0] + '</button><button class="sl-next">' + N.navText[1] + "</button>"), c = ot("<div>").addClass("sl-counter").html('<span class="sl-current"></span>/<span class="sl-total"></span>'), m = !1, V = 0, v = 0, x = ot("<div>").addClass("sl-caption " + N.captionClass + " pos-" + N.captionPosition), _ = ot("<div>").addClass("sl-image"), b = ot("<div>").addClass("sl-wrapper").addClass(N.className), tt = function(s) {
            if (K.length) {
                var l = new Image
                ...
                at(0, 0, 1),
                ot(l).on("error", function(t) {
                    $.eq(V).trigger(ot.Event("error.simplelightbox")),
                    U = !(m = !1),
                    g.hide();
                    var e = 1 == s || -1 == s;
                ...
                l.onload = function() {
                    void 0 !== s && $.eq(V).trigger(ot.Event("changed.simplelightbox")).trigger(ot.Event((1 === s ? "nextDone" : "prevDone") + ".simplelightbox")),
                    N.history && (f ? p = setTimeout(h, 800) : h()),
                    ...
                    N.loop || (0 === V && ot(".sl-prev").hide(),
                    V >= $.length - 1 && ot(".sl-next").hide(),
                    0 < V && ot(".sl-prev").show(),
                    V < $.length - 1 && ot(".sl-next").show()),
                    1 == $.length && ot(".sl-prev, .sl-next").hide(),
                    1 == s || -1 == s) {
                        var o = {
                            opacity: 1
                        };
                        N.animationSlide && (B ? (et(0, 100 * s + "px"),
                        setTimeout(function() {
                            et(N.animationSpeed / 1e3, "0px")
                        }, 50)) : o.left = parseInt(ot(".sl-image").css("left")) + 100 * s + "px"),
                        ot(".sl-image").animate(o, N.animationSpeed, function() {
                            m = !1,
                            y(n, t)
                        })
                    } else
            ...
            ot(st).on("resize." + G, tt),
            ot(lt).on("click." + G + " touchstart." + G, ".sl-close", function(t) {
                t.preventDefault(),
                U && it()
            }),
            ...
            J.on("click." + G, "button", t(function(t) {
                t.preventDefault(),
                L = 0,
                nt(ot(this).hasClass("sl-next") ? 1 : -1)
            }, N.throttleInterval));
            var e, a, n, i, o, s, l, r, c, h, p, d, u, f, g, m, v, x, b, y, C, w, T, E, S, M, I, k = 0, q = 0, X = 0, D = 0, Y = !1, A = !1, O = 0, R = !1, W = Z(1, 1, N.maxZoom), F = !1;
            _.on("touchstart." + G + " mousedown." + G, function(t) {
                ...
                "touchmove" == (t = t.originalEvent).type) {
                    ...
                    E = (o * y - e) / 2,
                    S = y - W,
                    x = s * y <= a ? 0 : Z(l - (C - n - a / 2 - l) / (y - S) * S, -1 * T, T),
                    b = o * y <= e ? 0 : Z(r - (w - i - e / 2 - r) / (y - S) * S, -1 * E, E),
                    at(x + "px", b + "px", y),
                    1 < y && (Y = !0,
                    ot(".sl-caption").fadeOut(200)),
                    u = M,
                    ...
                    r = b)) : (T = (s * (y = W) - a) / 2,
                    E = (o * y - e) / 2,
                    x = s * y <= a ? 0 : Z(f - (c - l), -1 * T, T),
                    b = o * y <= e ? 0 : Z(g - (h - r), -1 * E, E),
                ...
                1 == W && (Y = !1,
                ot(".sl-caption").fadeIn(200)),
                u = null,
                R = !1) : 1 === I ? (c = t.touches[0].clientX,
                h = t.touches[0].clientY) : 1 < I && (u = null)),
                A) {
                    var e = !(A = !1);
                    N.loop || (0 === V && L < 0 && (e = !1),
                    V >= $.length - 1 && 0 < L && (e = !1)),
                ...
                at("0px", "0px", W),
                Y = !1,
                ot(".sl-caption").fadeIn(200)) : (W = N.doubleTapZoom,
                z(0, 0, W),
                at("0px", "0px", W),
                ot(".sl-caption").fadeOut(200),
                Y = !0),
            ...
            ot("<img />").attr("src", $.eq(t).attr(N.sourceAttr)).on("load", function() {
                -1 == d.indexOf(ot(this).attr("src")) && d.push(ot(this).attr("src")),
                $.eq(V).trigger(ot.Event("nextImageLoaded.simplelightbox"))
            }),
            ot("<img />").attr("src", $.eq(e).attr(N.sourceAttr)).on("load", function() {
                -1 == d.indexOf(ot(this).attr("src")) && d.push(ot(this).attr("src")),
                $.eq(V).trigger(ot.Event("prevImageLoaded.simplelightbox"))
            })
        }, nt = function(e) {
            ...
            if (!(m || (t < 0 || t >= $.length) && !1 === N.loop)) {
                ...
                N.animationSlide && (B ? et(N.animationSpeed / 1e3, -100 * e - L + "px") : a.left = parseInt(ot(".sl-image").css("left")) + -100 * e + "px"),
                ot(".sl-image").animate(a, N.animationSpeed, function() {
                    setTimeout(function() {
                        var t = $.eq(V);
                        K.attr("src", t.attr(N.sourceAttr)),
                        -1 == d.indexOf(t.attr(N.sourceAttr)) && g.show(),
                        ot(".sl-caption").remove(),
                        tt(e),
                        N.preloading && w()
                    }, 100)
                })
            ...
            if (!m) {
                var t = $.eq(V)
                  , e = !1;
                t.trigger(ot.Event("close.simplelightbox")),
                N.history && (o ? history.pushState("", lt.title, s.pathname + s.search) : s.hash = "",
                clearTimeout(p)),
                ot(".sl-image img, .sl-overlay, .sl-close, .sl-navigation, .sl-image .sl-caption, .sl-counter").fadeOut("fast", function() {
                    N.disableScroll && T("show"),
                    N.htmlClass && "" != N.htmlClass && ot("html").removeClass(N.htmlClass),
                    ot(".sl-wrapper, .sl-overlay").remove(),
                    J.off("click", "button"),
                    ot(lt).off("click." + G, ".sl-close"),
                    ot(st).off("resize." + G),
                    ot(st).off("hashchange." + G),
                    e || t.trigger(ot.Event("closed.simplelightbox")),
                    e = !0
                }),
                K = ot(),
                m = U = !1
            ...
            if ("hide" == t) {
                ...
                if (lt.body.clientWidth < a) {
                    ...
                    ot(lt.body)[0].removeChild(i),
                    ot("body").data("padding", o),
                    0 < e && ot("body").addClass("hidden-scroll").css({
                        "padding-right": o + e
                    })
                }
            } else
                ot("body").removeClass("hidden-scroll").css({
        ...
        this.destroy = function() {
            ot(lt).off("click." + G).off("keyup." + G),
            it(),
            ot(".sl-overlay, .sl-wrapper").remove(),
            this.off("click")
        }
        ,
        this.refresh = function() {
            this.destroy(),
            ot(this).simpleLightbox(N)
        }
        ,
        this
    }
